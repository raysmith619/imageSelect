...
from select_mover import SelectMover, SelectMove
...
class SelectRegion(object):
    ...
    def get_corner_part(self, x, y):
        ...
        for corner in self.parts:
            if not corner.is_corner():
                continue
            p1c1x,p1c1y,p1c3x,p1c3y = corner.get_corner_rect()
    ...
    def on_motion(self, event):
        ...
        if self.has_selected():
            ...
            for part in parts:
                ...
                SlTrace.lg("motion on(%s) at xy=(%d,%d) by xinc=%d yinc=%d"
                       % (part.part_type, x,y, xinc, yinc), "movement")
        ...
        if len(parts) > 0:
            ncheck = 3
            if len(parts) > ncheck:
                SlTrace.lg("on parts(%d) > %d " % (len(parts), ncheck), "movement")
                for pa in parts:
                    SlTrace.lg("part: %s" % pa, "movement")
                SlTrace.lg("", "movement")
            self.highlight_clear()              # First clear all highlighted parts
            for part in parts:
                if not part.is_region():
                    SlTrace.lg("motion over %s" % part, "movement")
    ...
    def move_corner(self, corner, xinc,  yinc):
        ...
        delta_y = yinc
        SlTrace.lg("move_edge: %s  by delta_x=%d,delta_y=%d"
               % (corner, delta_x, delta_y), "movement")
        mover = SelectMover(self, delta_x=delta_x, delta_y=delta_y)
    ...
    def record_move_display(self):
        ...
        for move in self.record_move_list:
            ...
            cx = (c1x+c3x)/2 + offset_x
            cy = (c1y+c3y)/2 + offset_y
            move.tag = self.display_text((cx, cy), text=disp_str)
    ...
    def record_move_setup(self):
        if len(self.record_move_list) > 0:        
            if self.show_moved:
                for move in self.record_move_list:
                    if hasattr(move, 'tag') and move.tag is not None:
                        self.canvas.delete(move.tag)     # delete move tag
                        move.tag = None
    ...
    def add_part(self, part):
        ...
        SlTrace.lg("add_part: %s" % part, "parts")
    ...
    def up (self, event):
        ...
        x,y = cnv.canvasx(event.x), cnv.canvasy(event.y)
        ###got = event.widget.coords (tk.CURRENT, x, y)
        SlTrace.lg("up at x=%d y=%d" % (x,y), "movement")
        if self.has_selected():
