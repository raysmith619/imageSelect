...
class SelectGameControl(SelectControlWindow):
    ...
    def _init(self, *args, **kwargs):
        ...
        if "title" not in kwargs:
            kwargs["title"] = "Game Control"
        if "control_prefix" not in kwargs:
            kwargs["control_prefix"] = SelectGameControl.CONTROL_NAME_PREFIX
...
if __name__ == '__main__':
    ...
    SlTrace.setProps()
    cF = SelectGameControl(None, title="SelectGameControl Testing", display=False)
    cF2 = SelectGameControl()
    cF2.control_display()
